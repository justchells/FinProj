...
def test():
  ...
  for p in (3, 6, 9, 12):
    ...
    for i in xrange(1, 101):
      ...
      ret_data = []
      for fund in fund_names:
        (investment, wealth, abs_return, ann_return) = stats_dict[fund]
        ret_data.append(ann_return)
        
      avg = numpy.mean(ret_data)
      if avg > max_avg:
        print 'found higher return %.4f at %.2f' % (avg, inc_factor)
        max_factor = inc_factor
        max_avg = avg
    
    stat = '%d months MA, %.2f, %.2f' % (p, max_factor, max_avg * 100.0)
    test_data.append(stat)
